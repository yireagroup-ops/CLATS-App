...
export default function App() {
  ...
  useEffect(() => {
    ...
    const logged = S.getSess();
    const onboarded = localStorage.getItem("clats_onboarded") === "true";

    if (logged && logged.type === "parent" && logged.email) {
      // Verify profile is still active on Supabase
      pullParentFromSupabase(logged.email)
        .then((fresh) => {
          if (fresh) {
            setParent(fresh);
        ...
        .catch((err) => {
          if (err?.message === "ACCOUNT_DELETED") {
            ...
            setParent(null);
            setView("welcome");
            alert("Your account was deleted from the cloud database. Returning to greeting screen.");
          } else {
            S.clearSess();
            setParent(null);
            setView("welcome");
            alert("Could not connect to the cloud database. Please try again later.");
          }
        });
    } else if (!onboarded) {
      setView("onboarding");
      ...
      const runSync = async () => {
        ...
        try {
          ...
          if (res && res.synced) {
            ...
            setDbConnected(true);
          } else if (res && res.code === "ACCOUNT_DELETED") {
            // Account deleted from cloud database, log out parent cleanly
            S.clearSess();
            setParent(null);
            setView("welcome");
            alert("Your account was deleted from the cloud database. Returning to greeting screen.");
  ...
  const handleParentAuth = (p: Parent) => {
    setParent(p);
    S.setSess({ type: "parent", email: p.email });
    setView("parentDashboard");
  };
  ...
  const activateChildMode = (c: Child) => {
    setActiveChild(c);
    setView("childApp");
    logSystemEvent("auth", "child_mode_entry", c.id, parent?.email || undefined, `${c.name} entered child mode`);
  };

  const verifyChildPin = () => {
    if (!pinTargetChild) return;
    if (enteredPin === pinTargetChild.pin) {
      const target = pinTargetChild;
      setPinTargetChild(null);
      activateChildMode(target);
    } else {
